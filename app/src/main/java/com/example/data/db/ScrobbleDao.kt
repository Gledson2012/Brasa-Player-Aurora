...
interface ScrobbleDao {
    ...
    @Query("DELETE FROM pending_scrobbles WHERE retryCount >= :maxRetries")
    suspend fun deleteExhaustedScrobbles(maxRetries: Int = 3): Int

    @Query("UPDATE pending_scrobbles SET retryCount = retryCount + 1, lastError = :error WHERE id = :id")
